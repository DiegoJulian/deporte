...
export function analyseClv(records) {
    ...
    const clean = records.filter((r) => typeof r.closingFairProbability === 'number')
        .map((r) => r.closingFairProbability * r.takenOdds - 1);
...
export function analyseMovement(openingOdds, currentOdds, threshold = 0.005) {
    const p0 = 1 / openingOdds;
    const p1 = 1 / currentOdds;
    const d = p1 - p0;
    const direction = Math.abs(d) < threshold ? 'QUIETA' : d > 0 ? 'ACORTA' : 'ALARGA';
    return {
