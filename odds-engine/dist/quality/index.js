...
export function oddsQualityScore(input, cfg) {
    const c = cfg.confidence;
    const reasons = [];
    const factors = {};
    factors.completeness = input.marketComplete ? 1 : 0.7;
    if (!input.marketComplete)
        reasons.push('Mercado incompleto: el margen no es observable.');
    if (input.overround === null) {
        ...
        if (input.overround > 0.10)
            reasons.push(`Margen alto: ${(input.overround * 100).toFixed(1)} %.`);
    }
    factors.books = c.booksFloor + (1 - c.booksFloor) * (1 - Math.exp(-Math.max(0, input.booksUsed - 1) / c.booksScale));
    if (input.booksUsed <= 1)
        reasons.push('Una sola casa: sin referencia externa.');
    if (input.dispersion === null)
        factors.dispersion = 0.85;
    else {
        factors.dispersion = 1 / (1 + (input.dispersion / c.dispersionReference) ** 2);
        if (input.dispersion > c.dispersionReference)
            reasons.push(`Dispersion entre casas de ${(input.dispersion * 100).toFixed(1)} puntos.`);
    }
    ...
    if (factors.freshness < 0.5)
        reasons.push(`Cuota de hace ${(input.ageMs / 60000).toFixed(0)} min: puede estar desfasada.`);
    ...
    if (Math.abs(mv) > c.movementReference / 2)
        reasons.push(`La linea se ha movido ${(Math.abs(mv) * 100).toFixed(1)} puntos desde la apertura.`);
