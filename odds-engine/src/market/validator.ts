...
export function bestOddsPerOutcome(market: Market): number[] {
  return market.outcomes.map((o) => Math.max(...o.quotes.map((q) => q.odds as number)));
}

export function validateMarket(market: Market, cfg: EngineConfig, bookmaker?: string): ValidationReport {
  const issues: ValidationIssue[] = [];
  const push = (code: string, severity: ValidationSeverity, message: string): void => { issues.push({ code, severity, message }); };
  ...
  for (const o of market.outcomes) {
    if (ids.has(o.id)) push('SALIDA_DUPLICADA', 'ERROR', `Salida repetida: ${o.id}`);
    ids.add(o.id);
  }
  ...
  const picked = market.outcomes.map((o) =>
    bookmaker === undefined
      ? Math.max(...o.quotes.map((q) => q.odds as number))
      : (o.quotes.find((q) => q.bookmaker === bookmaker)?.odds as number | undefined),
  );
  ...
  if (overround < 0) {
    push('MARGEN_NEGATIVO', 'ERROR',
      `Suma de probabilidades ${(S * 100).toFixed(2)} % por debajo del ${(target * 100).toFixed(0)} % que le corresponde a este mercado: arbitraje dentro de la misma casa. Es un error palpable de precio, no una oportunidad: las condiciones de la casa permiten anular la apuesta.`);
